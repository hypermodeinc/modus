...
export default class SDKRemoveCommand extends Command {
  ...
  async run(): Promise<void> {
    ...
    if (args.version.toLowerCase() === "all") {
      if (!flags.force && !(await this.confirmAction("Really, remove all Modus SDK versions? [y/n]"))) {
        clearLine();
        return;
      }
    ...
    if (!flags.force && !(await this.confirmAction(`Really, remove Modus SDK version ${args.version}? [y/n]`))) {
      clearLine();
      return;
    }
