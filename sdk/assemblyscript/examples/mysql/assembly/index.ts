...
export function getAllPeople(): Person[] {
  ...
  const response = mysql.query<Person>(connection, query);
...
export function getPeopleByName(name: string): Person[] {
  ...
  const response = mysql.query<Person>(connection, query, params);
  return response.rows;
}
...
export function addPerson(name: string, age: i32): Person {
  ...
  const response = mysql.execute(connection, query, params);
