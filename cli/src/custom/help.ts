...
export default class CustomHelp extends Help {
  ...
  override formatCommands(commands: Command.Loadable[]): string {
    ...
    for (const command of commands) {
      ...
      const aliases =
        command.aliases.length > 0 ? chalk.dim(" (" + command.aliases.join("/") + ")") : ""

      out +=
        getMessageWithPad(name, description + aliases, FIRST_PAD, SECOND_PAD, rawName.length) + "\n"
    }
    return out.trim()
  }
    ...
    if (flags.length) {
      ...
      for (const flag of Object.values(command.flags)) {
        ...
        if (flag)
          this.log(
            getMessageWithPad(
              chalk.bold.blueBright(flagOptions),
              desc,
              FIRST_PAD,
              SECOND_PAD,
              flagOptions.length
 ...
 */
function getMessageWithPad(
  formattedFirstMsg: string,
  formattedSecondMsg: string,
  firstPad: number,
  secondPad: number,
