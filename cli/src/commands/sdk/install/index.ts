...
export default class SDKInstallCommand extends BaseCommand {
  ...
  static examples = [
    "modus sdk install assemblyscript",
    "modus sdk install assemblyscript v1.2.3",
    "modus sdk install",
    "modus sdk install --prerelease",
  ]
  ...
  private async getRuntimeVersions(
    sdks: { sdk: SDK; version: string }[],
    prerelease: boolean
  ): Promise<string[]> {
    ...
    for (const { sdk, version } of sdks) {
      ...
      await withSpinner(
        chalk.dim("Checking for latest runtime compatible with " + sdkText),
        async (spinner) => {
          const runtimeVersion = await vi.findLatestCompatibleRuntimeVersion(
            sdk,
            version,
            prerelease
          )
  ...
  private async installRuntime(version: string, force: boolean): Promise<void> {
    ...
    const installRuntime = await withSpinner(
      chalk.dim("Checking for runtime installation."),
      async (spinner) => {
