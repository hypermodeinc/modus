...
export default class SDKRemoveCommand extends BaseCommand {
  static args = {
    ...
    version: Args.string({
      description:
        "SDK version to remove, if removing a specific SDK. Leave blank to remove all versions of the SDK.",
      default: "all",
    }),
  }
  ...
  async run(): Promise<void> {
    try {
      ...
      if (!args.name) {
        ...
        const sdkVersionToRemove = await inquirer.select({
          ...
          choices: sdkVersions.map((item) => ({
            name:
              item.version === "all"
                ? chalk.bold(`All ${item.sdk} SDK versions`)
                : `${item.sdk} SDK ${item.version}`,
            value: item,
          })),
        })
      ...
      if (!args.version) {
        this.logError(
          `No SDK specified! Run ${chalk.whiteBright("modus sdk remove <name> [version]")}, or ${chalk.whiteBright("modus sdk remove all")}`
        )
        return
      }
