...
export default class DevCommand extends BaseCommand {
  ...
  static flags = {
    runtime: Flags.string({
      char: "r",
      description:
        "Modus runtime version to use. If not provided, the latest runtime compatible with the app will be used.",
    }),
    prerelease: Flags.boolean({
      char: "p",
      aliases: ["pre"],
      description:
        "Use a prerelease version of the Modus runtime. Not needed if specifying a runtime version.",
    }),
  ...
  async run(): Promise<void> {
    ...
    if (!(await fs.exists(path.join(appPath, MANIFEST_FILE)))) {
      ...
      this.log(
        chalk.red(
          "Please either execute the modus command from the app directory, or specify the path to the app you want to run."
        )
      )
      this.exit(1)
    }
    ...
    if (runtimeVersion) {
      ...
      if (!(await vi.runtimeVersionIsInstalled(runtimeVersion))) {
        if (await isOnline()) {
          await withSpinner(
            chalk.dim("Downloading and installing " + runtimeText),
            async (spinner) => {
              try {
          ...
          this.logError(
            `${runtimeText} is not installed, and you are offline. Please try again when you have an internet connection.`
          )
          this.exit(1)
        }
      }
      ...
      const version = await vi.findCompatibleInstalledRuntimeVersion(
        sdk,
        sdkVersion,
        flags.prerelease
      )
      if (!version) {
        this.logError(
          "Could not find a compatible Modus runtime version. Please try again when you have an internet connection."
        )
        return
      }
      runtimeVersion = version
